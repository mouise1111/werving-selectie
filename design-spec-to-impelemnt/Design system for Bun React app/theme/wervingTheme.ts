...
/**
 * Werving & Selectie brand theme for Astryx.
 *
 * Drop this file in your Bun app (e.g. src/theme/wervingTheme.ts), then:
 *
 *   import {Theme} from '@astryxdesign/core';
 *   import {wervingTheme} from './theme/wervingTheme';
 *   <Theme theme={wervingTheme} mode="light"><App /></Theme>
 *
 * For production, run: bunx @astryxdesign/cli theme build ./src/theme/wervingTheme.ts
 * and import the generated .js + .css instead.
 *
 * The lime accent is deliberately paired with deep-teal foreground text, so the
 * accent is seeded as a [light, dark] tuple rather than a raw token override
 * (a --color-accent override would leave --color-on-accent stale).
 */
export const wervingTheme = defineTheme({
  ...
  components: {
    // Every actionable element in this brand is a full pill.
